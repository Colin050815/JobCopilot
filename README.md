...
每条招呼语都基于该岗位的 JD 和你的简历生成，按“身份背景 → 技术方向 → JD 匹配技能 → 项目/实习证据 → 沟通意愿”组织，让 HR 快速看到真实匹配点。
...
- 🏠 **首页推荐投递**：可读取 BOSS 首页“精选职位”，强制用目标关键词与简历筛掉陪玩等无关岗位；当前批次不足时自动刷新并按岗位 ID 去重
...
- ✍️ **高信息密度招呼语**：每个岗位独立提取简历与 JD 的交集，写入身份、方向、技能、项目和实习证据；低质量草稿会自动重写一次
...
如需投递首页推荐岗位，先在 BOSS 首页确认“精选职位”已经显示，再把 **岗位来源** 改为 **BOSS 首页推荐（强制 AI 筛选）**。填写目标岗位关键词并准备简历文字后开始收集；方向无关的推荐岗位会显示为不可勾选，不会进入投递批次。当前推荐数量不足时，插件会根据目标数量与首批岗位数自动计算刷新轮数（硬上限 24 次），按岗位 ID 去重，并在连续两轮没有新岗位时提前停止；城市输入仍可编辑或清空，但不会参与首页推荐筛选。
